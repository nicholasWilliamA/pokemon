...
export const MultiplicationsScreen: React.FC<HomeNavigationStackScreenProps<'MultiplicationsScreen'>> = ({navigation}) => {
    ...
    function onSubmit(form: NewAdditionForm) {
        navigation.navigate('MultiplicationsCalculationScreen', {
    ...
    return (
        <View className="flex-1 bg-white">
            ...
            <View className="mt-6">
                <Text className="ml-2 font-bold">Minimal perkalian</Text>
                <Controller control={control} name="minimumNumber" render={({field: {onChange, value}}) => (
                    <OutlinedTextInput
                        keyboardType="number-pad"
                        placeholder="Masukkan angka minimal perkalian"
                        onChangeText={onChange}
                        value={value}
                    />
                )}/>
            ...
            <View className="mt-6">
                <Text className="ml-2 font-bold">Maksimal perkalian</Text>
                <Controller control={control} name="maximumNumber" render={({field: {onChange, value}}) => (
                    <OutlinedTextInput
                        keyboardType="number-pad"
                        placeholder="Masukkan angka maksimal perkalian"
                        onChangeText={onChange}
                        value={value}
                    />
                )}/>
                <Text className="ml-2 mt-2 text-slate-400">* angka ini akan ditambahkan dengan minimal perkalian</Text>
